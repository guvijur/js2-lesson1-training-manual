...
let tomas = new User1("Томas", 26);
console.log(tomas._age); // undefined - _age - локальная переменная
console.log(tomas.getAge()); // 26
tomas.setAge(32);
console.log(tomas.getAge()); // 32
tomas.setAge("54"); // Недопустимое значение
...
//
// Всё было красиво, пока я не прочитал это:
// Инкапсуляция – важная часть объектно-ориентированного программирования, чтобы данные в разных 
// экземплярах класса были отделены друг от друга; это реализуется в JavaScript с помощью оператора this. 
// Однако, в отличие от других объектно-ориентированных языков программирования, JavaScript не ограничивает 
// доступ к данным внутри экземпляра класса.
// Тут моё сознание рухнуло, как карточный, мать его, домик!!!
// В примере выше мы вместо доступного повсеместно свойства создали внутри ФК переменную "let _age", тем самым
// мы ограничили область видимости для этой переменной функцией. Т.е., инкапсулировали получается.
// И тут я читаю, что инкапсуляция в JS реализуется с помощью оператора this. 
// 
// WTF!?!?!?
//
// Что написано в методичке про Полиморфизм? Ну, как бы, ничего хорошего, я опять сразу не понял смысла.
// Полиморфизм в JS также завязан на prototype. Все методы и свойства в классе определяются через свойство
// prototype, а различные классы могут определять методы с одинаковыми именами. Проще говоря, у всех 
// объектов в JS есть первичный предок – класс Object. От него все классы наследуются по умолчанию. 
// При наследовании создаются классы, например, Car и Plane, но и у Car, и у Plane скорее всего будет 
// метод move(), который будет описывать перемещение. Однако поведение Car и Plane при выполнении метода
// move() разное. Это и есть полиморфизм.
// Немного поскрипев мозгами, я понял это так:
// Например, есть некая общая функция контруктор. На базе этой функции созданы N объектов.
// У каждого объекта в prototype создан метод, например, move, который определяет перемещение объекта.
// При этом, метод содержит разные свойства, разное поведение. И полиморфизм здесь заключается именно в том, 
// что объекты используют не один и тот же метод, а каждый свой метод с одинаковым названием.
